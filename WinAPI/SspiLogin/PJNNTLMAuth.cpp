...
long CNTLMClientAuth::NTLMAuthenticate(LPCTSTR pszUserName, LPCTSTR pszPassword)
{
...
long CNTLMClientAuth::DoNTLMAuthentication(LPCTSTR pszUserName, LPCTSTR pszPassword, LPCTSTR pszDomain)
{
    BOOL fDone = FALSE;
    DWORD cbIn = 0;
    BYTE InBuf[12000];
    ...
    long ss = GenClientContext(NULL, 0, OutBuf, &cbOut, &fDone, pszUserName, pszPassword, pszDomain);
    if (!SEC_SUCCESS(ss))
        return ss;
...
SECURITY_STATUS CNTLMClientAuth::GenClientContext(BYTE* pIn, DWORD cbIn, BYTE* pOut, DWORD* pcbOut, BOOL* pfDone, LPCTSTR pszUserName, LPCTSTR pszPassword, LPCTSTR pszDomain)
{
    ...
    PNEGOTIATE_MESSAGE NegMessage = (PNEGOTIATE_MESSAGE)new BYTE[OutBuffDesc.pBuffers->cbBuffer];

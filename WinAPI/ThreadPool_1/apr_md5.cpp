...
static int md5_update_buffer(apr_md5_ctx_t *context,
                                      ...
                                      int xlate_buffer)
{
    const unsigned char *input = (const unsigned char *)vinput;
...
bool apr_password_validate(const char *passwd, const char *hash)
{
    ...
    if (hash[0] == '$') {
        ...
        else if (!strncpy((char*)hash, apr1_id, strlen(apr1_id))) {
            /*
    ...
    return (strncmp(sample, hash, strlen(hash)) == 0) ? true : false;
}

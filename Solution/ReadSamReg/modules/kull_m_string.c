...
{
	int unicodeTestFlags = IS_TEXT_UNICODE_STATISTICS;
	return ((pUnicodeString->Length == sizeof(wchar_t)) && IsCharAlphaNumeric(((LPWSTR)pUnicodeString->Buffer)[0])) || IsTextUnicode((LPWSTR)pUnicodeString->Buffer, pUnicodeString->Length, &unicodeTestFlags);
}
...
{
	if (String->Buffer)
		String->Buffer = (DWORD64)((LONG64)(String->Buffer) + ((relative ? -1 : 1) * (ULONG_PTR)(BaseAddress)));
}
...
BOOL kull_m_string_copyUnicodeStringBuffer64(PLSA_UNICODE_STRING64 pSource, PUNICODE_STRING pDestination)
{
	...
	if (pSource && pDestination && pSource->MaximumLength && pSource->Buffer)
	{
		...
		pDestination->MaximumLength = pSource->MaximumLength;
		if (pDestination->Buffer = (PWSTR)LocalAlloc(LPTR, pSource->MaximumLength))
		{
			status = TRUE;
			RtlCopyMemory(pDestination->Buffer, (PWSTR)pSource->Buffer, pSource->MaximumLength);
		}
	}
	return status;
}
...
void kull_m_string_clearUnicodeString(PUNICODE_STRING pString)
{
	if(pString)
	{
		...
		kull_m_string_freeUnicodeStringBuffer(pString);
	}
}
...
void kull_m_string_displayGUID(IN LPCGUID pGuid)
{
	...
	if(Ptr_RtlStringFromGUID && NT_SUCCESS(Ptr_RtlStringFromGUID(pGuid, &uString)))
	{
		kprintf(L"%wZ", &uString);
		if(Ptr_RtlFreeUnicodeString)
			Ptr_RtlFreeUnicodeString(&uString);
	}
}
...
PWSTR kull_m_string_getRandomGUID()
{
	...
	{
		if(Ptr_RtlStringFromGUID && NT_SUCCESS(Ptr_RtlStringFromGUID(&guid, &uString)))
		{
...
void kull_m_string_LsaUnicode64ToLsaUnicode(PLSA_UNICODE_STRING64 pSource, PUNICODE_STRING pDst)
{
	if (pSource && pDst)
	{
		pDst->Length = pSource->Length;
		pDst->MaximumLength = pSource->MaximumLength;
		pDst->Buffer = (LPWSTR)pSource->Buffer;
	}
}